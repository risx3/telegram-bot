...
- [ ] Bot is running (`docker compose up -d` or `uv run python -m bot.main`)
...
### Option A — SMS Forwarder app (recommended)

1. Install **SMS Forwarder** from the Play Store.
...
2. Configure a forwarding rule:
   ...
   - **Filter**: only forward SMS from bank senders (e.g. `HDFCBK`, `ICICIB`, `SBIINB`)

3. Test the rule — the app usually has a "Test" button.
...
2. Copy the **Transaction ID / UTR / Ref No** — looks like `123456789012` (12 digits) or `HDFC12345678` (alphanumeric)
...
docker compose logs -f bot | grep -i sms
...
### Manually insert a transaction (skip SMS entirely)
